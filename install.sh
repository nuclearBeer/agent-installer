...
sudo sed -i.bak 's/api_key=.*/api_key=$api_key/' /etc/data_exim_collector/collector.conf
sudo sed -i.bak 's/secret_key=.*/secret_key=$secret_key/' /etc/data_exim_collector/collector.conf
